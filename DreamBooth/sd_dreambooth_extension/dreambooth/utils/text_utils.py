...
# 实现来自 https://github.com/bmaltais/kohya_ss
def encode_hidden_state(text_encoder: CLIPTextModel, input_ids, pad_tokens, b_size, max_token_length,
    ...
    if clip_skip <= 1:
        ...
        # 返回所有中间层的隐藏状态，并将所有只以字典的方式返回出来
        enc_out = text_encoder(input_ids, output_hidden_states=True, return_dict=True)
        # 将 enc_out 键值对列表里最后一个 hidden_states 的值返回回来，返回隐藏状态
        encoder_hidden_states = enc_out['hidden_states'][-clip_skip]
        # 对最后的隐藏值进行标准化，增强模型的泛化能力
        encoder_hidden_states = text_encoder.text_model.final_layer_norm(encoder_hidden_states)
        
    # 如果不填充 tokens 则直接返回隐藏状态
    if not pad_tokens:
