...
def generate_classifiers(
        args: DreamboothConfig,                 # Dreambooth接收的参数
        class_gen_method: str = "Native Diffusers",
        accelerator: Accelerator = None,        # 分布式并行计算用的
        ui=True,
        pbar: mytqdm = None
):
    """

    @param args: DreamboothConfig
    @param class_gen_method
    @param accelerator:一个可选的现有加速器。
    @param ui:它是由ui调用的，还是在训练期间运行的。
    @param pbar:进度条。
    @return:
    generated:生成的图像数量
    images:图像或图像路径的列表,取决于是否返回UI。
    如果ui为False,这将返回表示类路径的第二个路径数组。
    """
    out_images = []
    instance_prompts = []
    class_prompts = []
    try:
        status.textinfo = "准备数据集…"
