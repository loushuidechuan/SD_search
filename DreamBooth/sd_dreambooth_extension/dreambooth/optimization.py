...
def get_optimizer(optimizer: str, learning_rate: float, weight_decay: float, params_to_optimize):
    try:
        if optimizer == "Adafactor":
            ...
            return Adafactor(
                params_to_optimize,         # 要进行优化的模型参数
                lr=learning_rate,           # 学习率
                clip_threshold=1.0,         # 梯度裁剪阈值
                decay_rate=-0.8,            # 学习率衰减数值
                weight_decay=weight_decay,  # 权重衰减（L1、L2）的系数
                relative_step=False,        # 是否使用相对步长控制学习率
                scale_parameter=True,       # 是否对学习率进行缩放
                warmup_init=False,          # 是否进行预热学习
            )
            ...
            return CAME(
                ...
                weight_decouple=True,       # 是否对权重衰减进行解耦
                fixed_decay=False,          # 是否使用固定的衰减值
                clip_threshold=1.0,
                ams_bound=False,            # 是否使用 AMSBound
            )

        elif optimizer == "8bit AdamW":
            from bitsandbytes.optim import AdamW8bit
            return AdamW8bit(
                params_to_optimize,         # 要进行优化的参数
                lr=learning_rate,           # 学习率
                weight_decay=weight_decay,  # 权重衰减率
                percentile_clipping=100,    # 百分裁剪位（裁剪高于裁剪位的梯度，防止梯度爆炸
                min_8bit_size=4096,         # 8位量化阈值
                block_wise=True,            # 是否对参数进行块状量化
                amsgrad=False,              # 是否使用 AMSGrad 变体
                is_paged=False,             # 是否进行分页量化
            )
